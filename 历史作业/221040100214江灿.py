...
if __name__ == "__main__":
    ...
    if len(data_section) == 8 and sys.argv[1][-1:] != ";" and "" in data_section:
        print("数据段中部多;号")
        a=a+1
    if sys.argv[1].find("QN") == -1:
        print("数据段缺QN")
        a=a+1
    if sys.argv[1].find("ST") == -1:
        print("数据段缺ST")
        a=a+1
    if sys.argv[1].find("CN") == -1:
        print("数据段缺CN")
        a=a+1
    if sys.argv[1].find("PW") == -1:
        print("数据段缺PW")
        a=a+1
    if sys.argv[1].find("MN") == -1:
        print("数据段缺MN")
        a=a+1
    if sys.argv[1].find("Flag") == -1:
        print("数据段缺Flag")
        a=a+1
    if sys.argv[1].find("CP") == -1:
        print("数据段缺CP")
        a=a+1
if  a==0 and b==[0]:
    print("数据段完整")
